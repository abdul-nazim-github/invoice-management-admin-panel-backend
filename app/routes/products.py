...
def bulk_delete_products():
    ...
    if not data or 'ids' not in data or not isinstance(data['ids'], list):
        return error_response(error_code='validation_error', message="Invalid request. 'ids' must be a list of product IDs.", status=400)
    ...
    try:
        ...
        return error_response(error_code='not_found', message="No matching products found for the provided IDs.", status=404)
    except Exception as e:
        return error_response(error_code='server_error', message=ERROR_MESSAGES["server_error"]["delete_product"], details=str(e), status=500)
