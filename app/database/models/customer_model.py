...
def create_customer(name, email=None, phone=None, address=None, gst_number=None, status="active"):
    conn = get_db_connection()
    with conn.cursor() as cur:
        cur.execute(
            """INSERT INTO customers (name, email, phone, address, gst_number, status)
                 VALUES (%s,%s,%s,%s,%s,%s)""",
            (name, email, phone, address, gst_number, status)
        )
        cid = cur.lastrowid
    conn.commit()
    conn.close()
...
def list_customers(q=None, status=None, offset=0, limit=20):
    ...
    with conn.cursor() as cur:
        cur.execute(f"SELECT SQL_CALC_FOUND_ROWS * FROM customers{where_sql} ORDER BY created_at DESC LIMIT %s OFFSET %s",
                    (*params, limit, offset))
        rows = cur.fetchall()
